...
import { SPECIAL_DAYS, SpecialDay } from './special-days.entity';
...
export class PlanService {
    ...
    async generatePlan(days: number, transportType: string = 'public_transport', locale: string = 'tr'): Promise<DailyPlan[]> {
        ...
        const allAvailablePois = await this.poiService.findAll(locale);
        ...
        for (let day = 1; day <= days; day++) {
            ...
            // Özel gün kontrolü ile filtrele (async)
            ...
            const specialDay = await this.poiService.getSpecialDay(date, locale);
